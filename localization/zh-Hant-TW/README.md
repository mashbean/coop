...
完整來源盤點已登記於 [`sources.tsv`](sources.tsv)。使用者指南、專案概覽、文件入口、整合指南、API 參考及第一批開發指南共 31 份來源已完成第一輪翻譯，剩餘 6 份為本機開發、架構、API authentication、資料倉儲、部署與完整目錄。所有內容在第二位語言審查者與必要的領域審查者確認前，不會標示為 `reviewed`。NCMEC 文件另需法律、兒少安全、資安與事件應變審查。
